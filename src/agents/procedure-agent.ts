...
export class ProcedureAgent extends BaseAgent {
  ...
  private async loadPhasesData(): Promise<any> {
    ...
    const dataPath = path.join(projectRoot, 'data', 'phases.json');
    const data = await fs.readFile(dataPath, 'utf-8');
    return JSON.parse(data);
  }
  ...
  private async generateSOP(
    ...
    // Robust phase lookup with visible agent logs
    const phases = Array.isArray(phasesData) ? phasesData : (phasesData.phases || []);
    this.log(`Loaded phases: ${phases.map((p: any) => p.id || p.phase || p.code).join(', ')}`);
    this.log(`Input phase: ${input.phase}`);
    let phase: any = undefined;
    if (input.phase !== undefined && input.phase !== null && String(input.phase).trim() !== '') {
      const asNumber = Number(input.phase);
      if (!Number.isNaN(asNumber)) {
        phase = phases.find((p: any) => Number(p.id) === asNumber || Number(p.phase) === asNumber);
      }
      if (!phase && typeof input.phase === 'string') {
        const normalized = input.phase.trim().toLowerCase();
        phase = phases.find((p: any) => (p.code && p.code.toLowerCase() === normalized) || (p.name && p.name.toLowerCase() === normalized) || String(p.id) === normalized);
      }
    }
    if (!phase && input.phase) {
      throw new Error(`Phase ${input.phase} not found. Available ids: ` + phases.map((p: any) => p.id || p.phase || p.code).join(', '));
    }
...
**Materials Required:**
${(phase.materials || phase.keyMaterials || []).map((m: string) => `- ${m}`).join('\n')}
...
${holdPoint ? `
**Inspection Criteria:**
${(holdPoint.inspectionCriteria || []).map((c: any) => typeof c === 'string' ? `- ${c}` : `- ${c.criterion}: ${c.acceptanceCriteria || ''}`).join('\n')}
