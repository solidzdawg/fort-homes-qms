...
```mermaid
graph TD
    GM[General Manager] --> QM[Quality Manager]
    GM --> PM[Production Manager]
    GM --> OM[Operations Manager]
    
    QM --> DC[Document Controller]
    QM --> QA[QA Inspectors]
    
    PM --> PS[Production Supervisors]
    PS --> PW[Production Workers]
    
    OM --> MM[Maintenance Manager]
    OM --> IM[Inventory Manager]
    
    style GM fill:#9C27B0,stroke:#7B1FA2,color:#fff
    style QM fill:#4CAF50,stroke:#2E7D32,color:#fff
...
quadrantChart
    ...
    General Manager: [0.4, 0.95]
    Quality Manager: [0.85, 0.85]
    Production Manager: [0.75, 0.7]
    QA Inspector: [0.9, 0.5]
    Production Staff: [0.8, 0.2]
```
...
flowchart TD
    subgraph Process["[PROCESS NAME]"]
        direction TB
        A["📦 Input"] --> B["⚙️ Process Step 1"]
        B --> C{"🔍 Decision Point?"}
        C -->|"Path A"| D["⚙️ Process Step 2A"]
        C -->|"Path B"| E["⚙️ Process Step 2B"]
        D --> F["📐 Quality Check"]
        E --> F
        F --> G{"✅ Pass?"}
        G -->|"Yes"| H["➡️ Output"]
        G -->|"No"| I["🔄 Corrective Action"]
        I --> B
    end
...
| IMP-003 | [Description] | ⏳ | [Role] | YYYY-MM-DD | H/M/L |
