...
```mermaid
pie title Regulatory Requirements Coverage
...
```mermaid
graph LR
    A[Regulatory Requirement] --> B{Mapped to QMS?}
    B -->|Yes| C{SOP Exists?}
    B -->|No| G[GAP IDENTIFIED]
    C -->|Yes| D{Records Generated?}
    C -->|No| G
    D -->|Yes| E{Evidence Retained?}
    D -->|No| G
    E -->|Yes| F[COMPLIANT]
    E -->|No| G
    G --> H[Create CAPA]
